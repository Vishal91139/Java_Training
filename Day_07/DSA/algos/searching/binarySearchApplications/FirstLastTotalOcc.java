...
public class FirstLastTotalOcc {
    public static int firstOcc(int[] arr, int key){
        ...
        while(s <= e){
            int mid = s+(e-s)/2;
            if(arr[mid] == key){
                // store the index and move to the extreme left for 1st Occurence
                ans = mid;
                e = mid - 1;
            }else if(arr[mid] < key){
                s = mid + 1;
    ...
    public static int lastOcc(int[] arr, int key){
        ...
        while(s <= e){
            int mid = s+(e-s)/2;
            if(arr[mid] == key){
                // store the index and move to the extreme right for last Occurence
                ans = mid;
                s = mid + 1;
            }else if(arr[mid] < key){
                s = mid + 1;
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Total Occurence Index: "+t);
    }
}
